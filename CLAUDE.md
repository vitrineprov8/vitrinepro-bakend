...
- **auth**: `POST /auth/register` (isCompany), `POST /auth/login`, OAuth Google/LinkedIn callbacks. Reset de senha = **mock** (gap B2/B14).
...
- **mail (B14, @Global)**: `MailService` envia e-mail transacional via **Resend** (REST por `fetch`, sem dependência nova). Se `RESEND_API_KEY` ausente → **modo STUB** (loga, não envia; não quebra o fluxo). Nunca lança. Métodos: `sendConsentRequest(to,nome,token)` (usado por B3), `sendWelcome`, `sendPasswordReset` (B2, pronto p/ ligar). Templates HTML inline em `mail/mail.templates.ts`. Links usam `FRONTEND_URL`. Página pública que recebe o clique: front `/consentimento/[token]` → `POST /public/candidate-consent/:token`.
...
- **companies / teams**: contas empresa, seats, membros (convite por token = gap B7).
...
- **B14** ✅ (backend): módulo `mail` (Resend) feito; consentimento de B3 já usa envio real. Falta: ligar B2 (reset) e B7 (convite) ao MailService; domínio verificado em prod.
