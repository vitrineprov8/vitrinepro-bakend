...
## Mapa de módulos (`src/`)
`auth` · `users` · `profile` · `education` · `cv` · `tags` · `portfolio` · `uploads` · `storage` · `search` · `vagas` · `vaga-applications` · `vaga-publish-ledger` · `pipeline-templates` · `hunter-interests` · `hunter-candidates` (B3) · `mail` (B14, **@Global**) · `process-share` · `plans` · `subscriptions` · `coupons` · `companies` · `teams` · `saved-vagas` · `saved-filters` · `seo` · `stats` · `seed` (só fora de produção).
...
- **B2** ✅ (backend+front): reset de senha real ligado (ver §Endpoints/auth). Falta: validar E2E completo (Andres precisa digitar a nova senha — Claude não o faz).
...
- **B2/B7 validados só até onde dá sem digitar senha:** 2026-07-05 — `POST /auth/forgot-password` disparado de verdade pela UI (`testeia@getnada.com`, conta real) → resposta genérica de sucesso confirmada visualmente; `/redefinir-senha/[token]` e `/convite/[token]` renderizam certo para token inválido (404 real do backend, não mock). **Não validado ainda** (exige ação do Andres): abrir o e-mail de reset e digitar a nova senha; criar um convite de time de verdade (precisa de conta com plano TEAM/ENTERPRISE, não existe em `qa-test-accounts.json`) e aceitar logado como o convidado.
